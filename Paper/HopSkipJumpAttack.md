...
ImageNet data set를 이용해 학습된 모델을 공격한 결과는 다음과 같다. 
...
## Discussion 
HopSkipJumpAttack은 2021기준 현존하는 Adversarial Attack 테크닉 중에서도 매우 강력한 공격 기법에 속하고
Decision-based attack이라는 매우 어려운 공격 상황에서 query-efficient한 알고리즘으로서 동작한다.
다만 HopSkipJumpAttack은 기존 Decision-based attack과 마찬가지로 boundary 근처에서 target model로 여러번의 쿼리를 날려야 하는 특징이 있어서 HopSkipJumpAttack을 알고있는 딥러닝 서비스 운영자라고 하면 탐지될 가능성이 있다.    
